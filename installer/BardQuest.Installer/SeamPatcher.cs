...
public static class SeamPatcher
{
    ...
    {
        var live = Path.Combine(managedDir, "Assembly-CSharp.dll");
        var backup = live + ".bardquest-bak";
        ...
        var target = module.GetType(TargetType)
            ?? throw new System.InvalidOperationException($"Type not found: {TargetType}");
        var onEnable = target.Methods.SingleOrDefault(m => m.Name == TargetMethod && m.Parameters.Count == 0)
            ?? throw new System.InvalidOperationException($"Method not found: {TargetType}::{TargetMethod}()");
        ...
        ModuleDefinition module, IAssemblyResolver resolver, string managedDir, TypeDefinition mainMenu)
    {
        ...
        var modPath = Path.Combine(managedDir, "BardQuest.Mod.dll");
        if (File.Exists(modPath))
        {
            ...
            var bootstrap = mod.GetType(BootstrapType)
                ?? throw new System.InvalidOperationException($"Type not found in BardQuest.Mod: {BootstrapType}");
        ...
        if (inModule != null)
            return module.ImportReference(inModule.Methods.Single(m => m.Name == BootstrapMethod));

        throw new System.InvalidOperationException("BardQuest.Mod.dll not found and no in-module Bootstrap.");
    }

    public static void Restore(string managedDir)
    {
        foreach (var backup in Directory.GetFiles(managedDir, "*.bardquest-bak"))
        {
            var live = backup[..^".bardquest-bak".Length];
